...
export class UplayLauncher extends GameLauncher<UPlayGame> {
  path: string;
  ...
  async getGameImageBase64(game: IGame<UPlayGame>): Promise<IGameImage> {
    ...
    return {
      portrait: await BaseImg.posterResizeImageFile(game.imgUrl),
    };
  }
  ...
  async getGames(): Promise<RunGame<UPlayGame>[]> {
    ...
    const games = els.filter(
      (el) =>
        el.root.space_id !== undefined && !el.root.third_party_platform?.name
    );
    ...
    return (
      await Promise.all(
        games.map<Promise<RunGame<UPlayGame>>>(async (el) => {
          const tmp: IGame<UPlayGame> = {
            ...
            imgUrl: el.root.thumb_image
              ? Path.join(
                  this.imgPath,
                  this.getTranslation(el, el.root.thumb_image, 'default')
                )
              : undefined,
            raw: el,
          };
          tmp.raw.root.defaultName = this.getTranslation(
            el,
            el.root.name,
            'default'
          );
      ...
      (a.raw.root.sort_string || a.name).localeCompare(
  ...
  private async getRaw(write = false) {
    const file = await fs.promises.readFile(
      Path.join(this.path, 'cache\\configuration\\configurations'),
      'utf8'
    );

    const splitter = /version: 2\.0.*$/gm;
    const invalid = /:[\s\r]*-/gm;
    const invalid2 = /^[a-zA-Z]{1,3}---$/gm;
    const out: string[] = [];
    let cur: string[] = [];
    for (const line of file.split('\n')) {
      const nLine = cleanString(line);
      const sl = splitter.test(nLine);
      const i1 = invalid.test(nLine);
      const i2 = invalid2.test(nLine);
      if (cur.length > 0 && (sl || nLine.startsWith('root:'))) {
        out.push(cur.join('\n'));
        cur = [];
      }
      if (
    ...
    if (write) {
      fs.writeFileSync(Path.join(__dirname, 'tmp.yml'), out.join('\n---\n'));
    }

    let stream: fs.WriteStream | null = null;
    if (write) {
      stream = fs.createWriteStream(Path.join(__dirname, 'out.json'));
      stream.write('[\n');
    }
  ...
  private getGameExeName(game: IGame<UPlayGame>): string | null {
    ...
    const offline =
      game.raw.root.start_game?.offline?.executables?.[0]?.path.relative ||
      null;
    const r = online || offline;
    if (r) {
      return Path.parse(r).base;
    }
    return null;
  }
  ...
  private async resolveGamePath(
    ...
    return SearchUtil.searchFileInFolder(
      Path.join(this.installPath, game.raw.root.defaultName),
      exe
    );
  }
}
