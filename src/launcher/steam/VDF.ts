...
function parse(text: string, o?: any): Record<string, any> {
  const options = {
    ...o,
    types:
      typeof o === 'boolean'
        ? o // backward compatibility with the old boolean param
        : typeof o === 'object' && 'types' in o
        ? o.types
        : true,
    arrayify: typeof o === 'object' && 'arrayify' in o ? o.arrayify : true,
    conditionals:
      typeof o === 'object' && 'conditionals' in o ? o.conditionals : undefined,
  };
  if (
    options.conditionals &&
    !Array.isArray(options.conditionals) &&
    typeof options.conditionals === 'string'
  )
  ...
  let line;
  let sublines: any;
  function getNextLine(): any {
    if (sublines && sublines.length) {
      let _subline = sublines.shift();
    ...
    if (_line === undefined) return false; // this is the end
  ...
  while ((line = getNextLine()) !== false) {
    ...
    if (expect_bracket) {
      throw new SyntaxError(
        `VDF.parse: invalid syntax on line ${
          i + 1
        } (expected opening bracket, empty unquoted values are not allowed):\n${line}`
      );
    }
    ...
    while (true) {
      ...
      if (m === null) {
        throw new SyntaxError(
          `VDF.parse: invalid syntax on line ${i + 1}:\n${line}`
        );
      }
      ...
      if (val === undefined) {
        ...
        if (
          ...
          undefined /* || typeof stack[stack.length-1][key] !== 'object' */
        ) {
          stack[stack.length - 1][key] = {};
        ...
        // exists already, is an object, but not an array
          ...
          !Array.isArray(stack[stack.length - 1][key])
        ) {
          if (options.arrayify) {
        ...
        // exists already, is an array of objects
          ...
          if (!options.arrayify)
            throw new Error(
              'VDF.parse: this code block should never be reached with arrayify set to false! [1]'
            );
          ...
          ); // push that new (last) object on stack
        }
        ...
        if (
          options.conditionals !== undefined &&
          Array.isArray(options.conditionals) &&
          m[9]
        ) {
          let conditionals = m[9];
          const single_cond_regex = new RegExp(
            '^(\\|\\||&&)?(!)?\\$([A-Z0-9]+)'
          );
          let ok = false;
          while (conditionals) {
            ...
            if (d === null || !d[3])
              throw new SyntaxError(
                `VDF.parse: encountered an incorrect conditional: ${conditionals}`
              );
            conditionals = conditionals.replace(d[0], '').trim(); // erase parsed fragment from the list
            const op = d[1];
            const not = d[2] && d[2] === '!';
            const cond = d[3];
            const includes = options.conditionals.indexOf(cond) !== -1;
        ...
        // exists already, is an array
          ...
          if (!options.arrayify)
            throw new Error(
              'VDF.parse: this code block should never be reached with arrayify set to false! [2]'
            );
...
function _dump(obj: any, options: any, level: any) {
  if (typeof obj !== 'object') {
    throw new TypeError(
      `VDF.stringify: a key has value of type other than string or object: ${typeof obj}`
    );
  }
